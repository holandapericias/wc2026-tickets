...
export default function ScraperPage() {
      ...
      if (Array.isArray(scans) && scans.length > 0) {
        ...
        if (Array.isArray(tickets)) {
          ...
          tickets.forEach((t: { id: string; game_num: number; match_name: string }) => {
            ticketMap.set(t.id, { ticket_id: t.id, game_num: t.game_num, match_name: t.match_name, last_scan: null, scan_count: 0 });
          });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-dark-card border border-dark-border rounded-lg p-4">
        <h3 className="text-sm font-semibold text-dark-muted uppercase mb-3">Per-Ticket Data Freshness</h3>
        {loading ? (
          <div className="text-dark-muted text-sm">Loading...</div>
        ) : ticketScans.length === 0 ? (
          <div className="text-dark-muted text-sm">No tickets found. Seed the database first.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {ticketScans.map((ts) => {
                  const fresh = ts.last_scan && (Date.now() - new Date(ts.last_scan).getTime()) < 24 * 60 * 60 * 1000;
                  return (
                    <tr key={ts.ticket_id} className="border-t border-dark-border">
                      <td className="px-3 py-2 font-mono text-dark-muted">G{ts.game_num}</td>
                      ...
                      <td className="px-3 py-2 text-center">
                        <span className={`inline-block w-2 h-2 rounded-full ${fresh ? "bg-green-400" : ts.last_scan ? "bg-yellow-400" : "bg-red-400"}`} />
                      </td>
                    </tr>
                  );
                })}
